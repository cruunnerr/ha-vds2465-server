...
from .const import DOMAIN, CONF_DEVICES, CONF_KEYS, EVENT_VDS_ALARM, EVENT_VDS_MONITORING, CONF_POLLING_INTERVAL, DEFAULT_POLLING_INTERVAL
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    keys_raw = entry.options.get(CONF_KEYS, {})
    
    # Enrich devices with latest keys from central storage
    devices_enriched = {}
    for ident, dev_data in devices_raw.items():
        enriched_dev = dev_data.copy()
        keynr = str(dev_data.get("keynr", "0"))
        if keynr in keys_raw:
            enriched_dev["key"] = keys_raw[keynr]
        devices_enriched[ident] = enriched_dev

    current_ident_nrs = {str(d["identnr"]) for d in devices_enriched.values()}
    ...
    devices_config_list = list(devices_enriched.values())
